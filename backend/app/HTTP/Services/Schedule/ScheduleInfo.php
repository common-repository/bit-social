...
class ScheduleInfo
{
    ...
    {
        $config = $this->config();
        $accounts = !empty($config['accounts']['accountIds']) ? $config['accounts']['accountIds'] : [];
    ...
    public function postFilterArgument()
    {
        ...
        $filter['tax_query'] = !empty($postFilters['categories_and_tags']) ? (new WpPostController())->categoryAndTags($postFilters['post_type'], $postFilters['categories_and_tags']) : '';
